...
<button onClick={(e)=>{e.stopPropagation();if(confirm("Delete '"+u.name+"'?")){setUploads(p=>p.filter((_,i)=>i!==uploads.indexOf(u)||uploads.filter((x,xi)=>xi<uploads.indexOf(u)).length!==j));setUploads(p=>p.filter(x=>x.name!==u.name||x.date!==u.date));}}} style={{padding:"3px 6px",borderRadius:4,fontSize:10,fontWeight:700,border:"1px solid #fca5a5",background:"#fef2f2",color:"#dc2626",cursor:"pointer"}}>🗑️</button>
</div>
</div>
</div>)}
</div>
